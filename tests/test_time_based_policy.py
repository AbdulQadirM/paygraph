...
class TestTimeBudgetPolicyBasics:
    ...
    def test_hourly_budget_enforcement(self):
        ...
        policy = SpendPolicy(
            max_transaction=100.0, daily_budget=1000.0, hourly_budget=50.0
        )
        engine = PolicyEngine(policy)

        # First transaction should pass
        with (
            patch("paygraph.time_periods.datetime") as mock_dt,
    ...
    def test_weekly_budget_enforcement(self):
        ...
        policy = SpendPolicy(
            max_transaction=200.0, daily_budget=1000.0, weekly_budget=200.0
        )
        engine = PolicyEngine(policy)
    ...
    def test_monthly_budget_enforcement(self):
        ...
        policy = SpendPolicy(
            max_transaction=1500.0, daily_budget=2000.0, monthly_budget=1500.0
        )
        engine = PolicyEngine(policy)
...
class TestBackwardCompatibility:
    ...
    def test_existing_daily_budget_still_works(self):
        ...
        policy = SpendPolicy(max_transaction=200.0, daily_budget=200.0)
        engine = PolicyEngine(policy)
    ...
    def test_existing_policy_fields_unchanged(self):
        ...
        assert not result.approved
        assert (
            "is not in the allowed list" in result.denial_reason
            or "is blocked" in result.denial_reason
        )
...
class TestEdgeCases:
    ...
    def test_period_boundary_transitions(self):
        ...
        policy = SpendPolicy(
            max_transaction=100.0, daily_budget=1000.0, hourly_budget=100.0
        )
        engine = PolicyEngine(policy)
...
class TestPolicyEngineIntegration:
    ...
    def test_policy_engine_summary_includes_time_periods(self):
        ...
        # Get summaries
        hourly_summary = engine._period_tracker.get_period_summary(
            "hourly", current_time
        )
        weekly_summary = engine._period_tracker.get_period_summary(
            "weekly", current_time
        )
